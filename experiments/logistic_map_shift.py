"""Logistic map periodic-to-chaotic regime-shift experiment for TAMC.

Goal:
    Show that topological attractor drift can detect a structural shift
    from periodic-ish to chaotic logistic-map dynamics while simple
    first-order statistics remain comparatively weak. This is Stage 1B:
    proving TAMC generalizes beyond sine/quasi-periodic signals to a
    classic nonlinear dynamical system with a controlled bifurcation.

Run:
    uv run python experiments/logistic_map_shift.py --seed 0
    uv run python experiments/logistic_map_shift.py --multi-seed 10
"""
...
def generate_logistic_map_regime_shift(
    ...
    x0_a = rng.uniform(0.2, 0.8)
    x0_b = rng.uniform(0.2, 0.8)
...
def compute_topological_drift(
    ...
    # Logistic map is a discrete recurrence; a short delay (2) keeps
    # successive embedding coordinates correlated with the underlying
    # period-doubling/chaotic structure, unlike the long delay (8) used
    # for the smoother sine/quasi-periodic signal.
    config = EmbeddingConfig(dimension=3, delay=2, window=window)
...
def plot_results(
    ...
    axes[0].set_title("Logistic map regime shift: periodic-ish to chaotic")
...
def run_experiment(
    ...
    series, shift_index = generate_logistic_map_regime_shift(seed=seed)
...
def run_single_seed(seed: int, figures_dir: Path) -> None:
    ...
    figure_path = figures_dir / "logistic_map_shift_drift.png"
    plot_results(
        series=raw["series"],
        times=raw["times"],
    ...
    metrics_path = figures_dir / "logistic_map_shift_metrics.csv"
...
def run_multi_seed(n_seeds: int, figures_dir: Path) -> None:
    ...
    figures_dir.mkdir(parents=True, exist_ok=True)
    summary_path = figures_dir / "logistic_map_shift_multiseed_metrics.csv"
