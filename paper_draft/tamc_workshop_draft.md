...
- **TAMC does not beat a simple statistical/distributional regime-similarity gate on real data.** The ETTh1-only result (modest TAMC lead, 0.0027 vs. autocorrelation's 0.0015) does not hold once a direct RG-style regime-similarity baseline and four more real datasets are added (Section 5.3): TAMC loses to the RG-style gate on all 5 datasets tested (ETTh1, ETTh2, ETTm1, ETTm2, Weather), never ranking better than 4th of 8 variants. This is the most direct evidence so far against topology being uniformly superior to simpler statistical alternatives on real data, in contrast to the clear, consistent advantage TAMC's topological drift shows over non-topological baselines on the synthetic detection tasks (Section 5.1).
