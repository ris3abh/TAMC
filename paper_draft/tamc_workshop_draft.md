...
**Does this generalize beyond sine to the other two detection systems?** `experiments/dynamical_forecast_adaptation.py` repeats the comparison on `logistic_map` and `lorenz` (same causal generators as Section 5.1), adding a second standalone adaptive forecaster, `RollingLinearARForecaster` (refits a small ridge-regularized AR model from the current context window alone at every prediction call), and gating both adaptive forecasters with TAMC as well as with mean/variance-, autocorrelation-, and spectral-drift under the identical control law from Section 3.3. 10-seed results (full table in `figures/dynamical_forecast_adaptation_tradeoff_summary.csv`):

| System | TAMC-gated blend (recent-pattern) | Always-on blend (recent-pattern) | Best variant overall |
|---|---|---|---|
...
- **Topology-gated adaptation does not generalize uniformly across the systems where TAMC detection already works.** Extending the forecast-adaptation comparison (Section 5.2) to logistic map and Lorenz gives a mixed result, not a clean generalization: TAMC-gated blending is the best variant on Lorenz, but on logistic map nothing helps (the frozen forecaster is already sufficient, though TAMC's gate is still the least-harmful non-frozen option), and on sine it is no longer the best choice once a stronger adaptive forecaster (`RollingLinearARForecaster`) is added to the comparison. The original sine-only result was not wrong given the variants it compared, but is not the full picture.
